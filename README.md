...
> 💡 **关于「机器人进群」**：NapCat 没有机器人**主动申请加群**的接口，所以机器人无法自己加群。正确做法是 **Owner 在 QQ 客户端把机器人邀请进群**——机器人会自动同意、登记为生效群，并私聊通知 Owner；非 Owner 邀请则转交 Owner 审批（`/yes`/`/no`）。
...
- **非 Owner 邀请转审批**：其他人邀请机器人进群时，机器人**不会自动拒绝**，而是私聊通知 Owner 审批（`/yes <ID>` 同意、`/no <ID>` 拒绝，同意后自动登记生效群）。若想直接拒绝，可在 `config.json` 设 `reject_non_owner_invites: true`。
...
- 🚪 **邀请进群通知**：Owner 邀请机器人入群后自动登记生效群并**私聊通知 Owner**；非 Owner 邀请转交 Owner 审批（NapCat 不支持主动加群）。
